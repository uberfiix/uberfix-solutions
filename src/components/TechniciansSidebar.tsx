...
const TechniciansSidebar = ({
  ...
  const baseTechnicians: Technician[] =
    dbTechnicians && dbTechnicians.length > 0
      ? dbTechnicians.map((t) => ({
          id: t.id,
          name: t.name,
          phone: t.phone,
      ...
      : mockTechnicians.filter(
          (t) => activeService === 'all' || t.specialty === activeService
        );
  ...
  const filteredTechnicians = baseTechnicians.filter((tech) => {
    ...
    const matchesAvailability =
      !showAvailableOnly || tech.status === 'available';
    return matchesSearch && matchesAvailability;
  });

  const availableCount = baseTechnicians.filter(
  ...
  const handleRequestService = (technicianId: string) => {
    ...
    if (onRequestService) {
      onRequestService(technicianId);
    } else {
      navigate(`/new-request?technician=${technicianId}`);
    }
  };
